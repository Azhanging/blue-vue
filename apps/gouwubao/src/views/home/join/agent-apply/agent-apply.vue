...
<template>
	<bv-home-view :router-level='3' style="background-color: #F4F4F4;">
      ...
      title:{
		...
		<div class="bc-row">
			<img :src="`${$config.path.static}/img/member/join/agent/join-banner.png`" alt="" class="bc-w-100">
		</div>
		...
		<div class="bc-row bc-f-16rp bc-bg-white">
			...
			<div class="bc-row bc-bd-b-e5e">
				<a class="bc-block bc-flex bc-flex-ai-c bc-pd-17rp" @click="openPickerWindows('level')">
					...
					<div class="bc-flex-2 bc-t-r bc-t-999 bc-t-hide">
						<template v-if="agent.level.slots[0].value">
							{{getJoinSelectName(agent.level.slots)}}
						</template>
						<template v-else>
			...
			<div class="bc-row">
				<a class="bc-block bc-flex bc-flex-ai-c bc-pd-17rp" @click="openPickerWindows('area')">
					...
					<div class="bc-flex-2 bc-t-r bc-t-999 bc-t-hide">
						<template v-if="agent.area.slots[0].value">
							{{getJoinSelectName(agent.area.slots)}}
						</template>
						<template v-else>
		...
		<div class="bc-row bc-mg-t-14rp ">
			...
			<div class="bc-row bc-bg-white bc-pd-17rp">
				...
				<div class="bc-row bc-mg-t-25rp bc-mg-l-10rp ">
					<span class="upload-img" v-for="(item,index) in business.imgs" :style="`background-image:url(${item});`">
						<i class="remove icon icon-close" @click="removeUpload($refs['business'],business.imgs,index)"></i>
					</span>
					<upload
						...
						:multiple="true"
						:upload-data="business.imgs"
						:maximum="5"
						style="font-size:0;"
						v-show="business.imgs.length < 5"
					>
			...
			<div class="bc-row bc-bg-white bc-pd-17rp bc-mg-t-10rp">
				...
				<div class="bc-row bc-mg-t-25rp bc-mg-l-10rp">
					<span class="upload-img" v-for="(item,index) in receipt.imgs" :style="`background-image:url(${item});`">
						<i class="remove icon icon-close" @click="removeUpload($refs['receipt'],receipt.imgs,index)"></i>
					</span>
					<upload
						accept="image/png,image/gif,image/jpeg,image/webp"
						input-id="input1"
						...
						@input-file="inputFile"
						:multiple="true"
						:upload-data="receipt.imgs"
						:maximum="5"
						style="font-size:0;"
						v-show="receipt.imgs.length < 5"
					>
						<span class="upload-img upload-img-add"></span>
					</upload>
				</div>
		...
		<!-- 使用本账号作为创客关联账号 -->
		<div class="bc-row  bc-bg-white bc-mg-t-14rp bc-f-16rp">
			<div class="bc-flex bc-flex-ai-c bc-pd-17rp bc-bd-b-e5e">
				<div class="bc-flex-4 bc-f-b">
					<div class="bc-flex bc-flex-ai-c">
						...
						<img :src="`${$config.path.static}/img/member/join/agent/tips.png`" alt="" style="width:1rem;height:1rem;" @click.stop="$toast({message:'您可输入任意点通宝账号作为您的创客账号'})">
					</div>
				</div>
				<div class="bc-flex-1 bc-t-r">
					<mt-switch v-model="bindChuankeMember.status" class="bc-flex-jc-e"></mt-switch>
				</div>
			</div>

			<div class="bc-flex bc-flex-jc-sb bc-flex-ai-c bc-pd-17rp" v-show="!bindChuankeMember.status">
				<div class="bc-flex-2 bc-f-b bc-f-16rp">
					<span>添加账号</span>
				</div>
				<div class="bc-flex-1 bc-t-999">
					<input type="text" class="bc-bd-none bc-t-r" placeholder="请输入手机号" v-model="bindChuankeMember.phone">
				</div>
			</div>
		</div>
		...
		<template slot="other">
			...
			<bv-transition transition-name="top">
				<bv-picker-windows
					ref="pickerWindows"
					v-show="pickerWindows.showStatus"
					:title="pickerWindows.data.title"
					:slots="pickerWindows.data.slots"
					:ajax="pickerWindows.data.ajax"
					:select="{
...
<script>
  ...
  function validateArea() {
    let i = 0;
    const pickerWindows = this.pickerWindows;
  ...
  export default {
    ...
    methods: {
      ...
      openPickerWindows(type) {
        this.$closeToast();
        const pickerWindows = this.pickerWindows;
        const currentPickerData = this.agent[type];
        if (type === 'area') {
          let levelPickerData = this.agent['level'].slots[0];
      ...
      submit() {
        ...
        const bindChuankeMember = this.bindChuankeMember;
        ...
        this.$axios({
          ...
          data: {
            ...
            licence: this.receipt.imgs.join('|'),
          ...
          if (status !== 4) {
            ...
            this.$router.push(`${this.$route.fullPath}/success`);
          }
        });
      }
    }
